...
function createAnimatedBackground() {
  try {
    ...
    // Animation settings
    const fps = 6; // 12 FPS as requested
    ...
    try {
      const scaleY = GAME_HEIGHT / 512;
      const scaleX = scaleY;
      const tileWidth = 2048 * scaleX;
      const numTiles = Math.ceil(LEVEL_WIDTH / tileWidth) + 1;
      ...
      // Final fallback to colored background
      add([
        rect(LEVEL_WIDTH, GAME_HEIGHT),
        pos(0, 0),
      ...
      console.log("Using colored background as final fallback");
    }
  }
}
